...
var hasTailwindClassName = ({ className: className$1, classPrefix, type }) => {
	if (!className$1) return false;
	if (type === "exact") {
		const split = className$1.split(" ");
	...
	return classPrefix.some((prefix) => {
		return className$1.startsWith(prefix) || className$1.includes(` ${prefix}`) || className$1.includes(`!${prefix}`) || className$1.includes(`:${prefix}`);
	});
};
var AbsoluteFillElementRefForwarding = (props, ref) => {
	const { style: style$1, ...other } = props;
...
var useResolvedVideoConfig = (preferredCompositionId) => {
	...
	const composition = compositions.find((c$2) => c$2.id === compositionId);
...
var useVideo = () => {
	...
	const selected = compositions.find((c$2) => {
		return canvasContent?.type === "composition" && c$2.id === canvasContent.compositionId;
	});
...
var getErrorStackWithMessage = (error$1) => {
	const stack = error$1.stack ?? "";
	return stack.startsWith("Error:") ? stack : `${error$1.message}
${stack}`;
};
var isErrorLike = (err) => {
...
function cancelRenderInternal(scope, err) {
	...
	if (isErrorLike(err)) {
		error$1 = err;
		if (!error$1.stack) error$1.stack = new Error(error$1.message).stack;
	} else if (typeof err === "string") error$1 = Error(err);
	else error$1 = Error("Rendering was cancelled");
	if (scope) scope.remotion_cancelledError = getErrorStackWithMessage(error$1);
	throw error$1;
}
...
var InnerComposition = ({ width, height, fps, durationInFrames, id, defaultProps, schema, ...compProps }) => {
	...
	if (typeof window !== "undefined") window.remotion_seenCompositionIds = Array.from(new Set([...window.remotion_seenCompositionIds ?? [], id]));
...
var checkMultipleRemotionVersions = () => {
	...
	if (alreadyImported) {
		if (alreadyImported === VERSION) return;
...
var createRuntimeValueStore = (initialSnapshot) => {
	...
	return {
		...
		setSnapshot: (newSnapshot) => {
			...
			for (const listener of listeners$1) listener(snapshot);
		}
	};
};
...
var getSequenceCropClipPath = ({ left, right, top, bottom, style: style$1 }) => {
	if (left === 0 && right === 0 && top === 0 && bottom === 0) return null;
	const serializeRadius = (radius) => typeof radius === "number" ? `${radius}px` : radius;
	const shorthand = serializeRadius(style$1?.borderRadius);
	const longhands = [
		style$1?.borderTopLeftRadius,
		style$1?.borderTopRightRadius,
		style$1?.borderBottomRightRadius,
		style$1?.borderBottomLeftRadius
	];
...
var usePremounting = ({ from, durationInFrames, premountFor, postmountFor, style: style$1, styleWhilePremounted, styleWhilePostmounted, hideWhilePremounted }) => {
	...
	return {
		...
		premountingStyle: (0, react.useMemo)(() => {
			if (!isPremountingOrPostmounting) return style$1;
			return {
				...style$1,
...
var angleUnits$1 = new Set([
	"deg",
	"rad",
	"grad",
	"turn"
]);
var lengthUnits$1 = new Set([
	"%",
	"cap",
	"ch",
	"cm",
...
var transformOriginKeywords$1 = new Set([
	"left",
	"center",
	"right",
	"top",
...
function spring$1({ frame: passedFrame, fps, config = {}, from = 0, to = 1, durationInFrames: passedDurationInFrames, durationRestThreshold, delay: delay$1 = 0, reverse = false }) {
	...
	const delayProcessed = (reverse ? (passedDurationInFrames ?? naturalDurationGetter.get()) - passedFrame : passedFrame) + (reverse ? delay$1 : -delay$1);
...
function modernColorCall$1(name) {
	return /* @__PURE__ */ new RegExp(name + "\\(\\s*(" + MODERN_VALUE$1 + ")\\s+(" + MODERN_VALUE$1 + ")\\s+(" + MODERN_VALUE$1 + ")(?:\\s*\\/\\s*(" + MODERN_VALUE$1 + "))?\\s*\\)");
}
function getMatchers$1() {
	...
	if (cachedMatchers.rgb === void 0) {
		cachedMatchers.rgb = /* @__PURE__ */ new RegExp("rgb" + call$1(NUMBER$1, NUMBER$1, NUMBER$1));
		cachedMatchers.rgba = /* @__PURE__ */ new RegExp("rgba" + call$1(NUMBER$1, NUMBER$1, NUMBER$1, NUMBER$1));
		cachedMatchers.hsl = /* @__PURE__ */ new RegExp("hsl" + call$1(NUMBER$1, PERCENTAGE$1, PERCENTAGE$1));
		cachedMatchers.hsla = /* @__PURE__ */ new RegExp("hsla" + call$1(NUMBER$1, PERCENTAGE$1, PERCENTAGE$1, NUMBER$1));
...
var withInteractivitySchema = ({ Component, componentName, componentIdentity = null, schema, supportsEffects }) => {
	...
	const Wrapped = (0, react.forwardRef)((props, ref) => {
		...
		const currentRuntimeValueDotNotation = (0, react.useMemo)(() => readValuesFromProps(cleanProps, flatKeys, flatSchema), flatKeys.map((key) => getRuntimeValueForSchemaKey({
			flatSchema,
			key,
			props: cleanProps
		})));
...
var useCropStyle = ({ cropLeft, cropRight, cropTop, cropBottom, style: style$1, componentName }) => {
	...
	return (0, react.useMemo)(() => {
		...
		if (cropClipPath === null) return style$1;
		return {
			...style$1,
...
var CanvasPool = class {
	constructor(width, height) {
		...
		this.lostContexts = /* @__PURE__ */ new Set();
		this.width = width;
		this.height = height;
	}
	getPair(backend) {
...
var CanvasRefForwardingFunction = ({ width, height, fit, className: className$1, style: style$1, effects, ...props }, ref) => {
...
var AnimatedImageContent = (0, react.forwardRef)(({ src, width, height, onError, loopBehavior = "loop", playbackRate = 1, fit = "fill", requestInit, effects, controls, ...props }, canvasRef) => {
	...
	(0, react.useLayoutEffect)(() => {
		...
		const delay$1 = delayRender2(`Rendering frame at ${currentTime} of <AnimatedImage src="${src}"/>`);
		let cancelled = false;
		imageDecoder.getFrame(currentTime, loopBehavior).then(async (videoFrame) => {
			if (cancelled) return;
			if (videoFrame === null) {
				ref.current?.clear();
				continueRender2(delay$1);
				return;
			}
			if (await ref.current?.draw(videoFrame.frame) && !cancelled) continueRender2(delay$1);
		}).catch((err) => {
			if (cancelled) return;
			if (onError) {
				onError(err);
...
var AnimatedImageInner = ({ src, width, height, onError, fit, playbackRate, loopBehavior, id, className: className$1, style: style$1, durationInFrames, from, premountFor, postmountFor, styleWhilePremounted, styleWhilePostmounted, cropLeft, cropRight, cropTop, cropBottom, requestInit, effects = [], controls, ref, ...sequenceProps }) => {
...
var Loop = ({ durationInFrames, times = Infinity, children, name, showInTimeline, ...props }) => {
	...
	const style$1 = props.layout === "none" ? void 0 : props.style;
...
var SolidInner = ({ color, width, height, effects = [], className: className$1, style: style$1, pixelDensity, overrideId, reference }) => {
...
var Solid = withInteractivitySchema({
	Component: (0, react.forwardRef)(({ effects = [], controls, color, height, width, className: className$1, durationInFrames, style: style$1, name, from, trimBefore, freeze, hidden, showInTimeline, pixelDensity, cropLeft, cropRight, cropTop, cropBottom, ...props2 }, ref) => {
		const memoizedEffectDefinitions = useMemoizedEffectDefinitions(effects);
		const actualRef = (0, react.useRef)(null);
		(0, react.useImperativeHandle)(ref, () => {
			return actualRef.current;
...
var HtmlInCanvasContent = (0, react.forwardRef)(({ width, height, effects, children, onPaint, onInit, pixelDensity, controls, style: style$1 }, ref) => {
...
var HtmlInCanvasInner = (0, react.forwardRef)(({ width, height, effects = [], children, onPaint, onInit, pixelDensity, controls, style: style$1, cropLeft, cropRight, cropTop, cropBottom, durationInFrames, name, ...sequenceProps }, ref) => {
	const memoizedEffectDefinitions = useMemoizedEffectDefinitions(effects);
	const actualRef = (0, react.useRef)(null);
...
var CanvasImageContent = (0, react.forwardRef)(({ src, crossOrigin, width, height, fit = "fill", effects, controls, onError, className: className$1, style: style$1, id, pauseWhenLoading, maxRetries = 2, delayRenderRetries, delayRenderTimeoutInMilliseconds, refForOutline, ...canvasProps }, ref) => {
	const { delayRender: delayRender2, continueRender: continueRender2, cancelRender: cancelRender2 } = useDelayRender();
	const { delayPlayback } = useBufferState();
	const [outputCanvas, setOutputCanvas] = (0, react.useState)(null);
...
var CanvasImage = withInteractivitySchema({
	Component: (0, react.forwardRef)(({ src, crossOrigin, width, height, fit, effects = [], className: className$1, style: style$1, id, onError, pauseWhenLoading, maxRetries, delayRenderRetries, delayRenderTimeoutInMilliseconds, durationInFrames, from, trimBefore, freeze, premountFor, postmountFor, styleWhilePremounted, styleWhilePostmounted, hidden, name, showInTimeline, cropLeft, cropRight, cropTop, cropBottom, controls, _remotionInternalDocumentationLink, _remotionInternalCropComponentName, outlineRef, ...canvasProps }, ref) => {
...
var IFrame = (0, react.forwardRef)(IFrameRefForwarding);
...
var NativeImgInner = ({ hidden, name, showInTimeline, src, from, trimBefore, durationInFrames, freeze, premountFor, postmountFor, style: style$1, styleWhilePremounted, styleWhilePostmounted, cropLeft, cropRight, cropTop, cropBottom, controls, outlineRef: refForOutline, ...props2 }) => {
...
var getFitFromObjectFit = (style$1) => {
	const objectFit = style$1?.objectFit;
	if (objectFit === "fill" || objectFit === "contain" || objectFit === "cover") return objectFit;
};
var ImgInner = ({ effects = [], ref, hidden, name, showInTimeline, src, from, trimBefore, durationInFrames, freeze, premountFor, postmountFor, styleWhilePremounted, styleWhilePostmounted, controls, width, height, className: className$1, style: style$1, cropLeft, cropRight, cropTop, cropBottom, id, pauseWhenLoading, maxRetries, delayRenderRetries, delayRenderTimeoutInMilliseconds, onImageError, ...props2 }) => {
	const refForOutline = (0, react.useRef)(null);
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CanvasImageWithPrivateProps, {
		...
		width: canvasWidth,
		height: canvasHeight,
		fit: getFitFromObjectFit(style$1) ?? "fill",
		effects,
		className: className$1,
		style: style$1,
		cropLeft,
...
var makeInteractiveElement = (tag, displayName, schema) => {
	const Inner = (0, react.forwardRef)((propsWithControls, ref) => {
		const { durationInFrames, from, trimBefore, freeze, hidden, name, showInTimeline, controls, cropLeft, cropRight, cropTop, cropBottom, style: style$1, ...props2 } = propsWithControls;
		const croppedStyle = useCropStyle({
			cropLeft,
			cropRight,
			cropTop,
...
var injectCSS = (css) => {
	...
	head.prepend(style$1);
	injected$1[css] = style$1;
	return () => {
...
var OffthreadVideoForRendering = ({ onError, volume: volumeProp, playbackRate, src, muted, allowAmplificationDuringRender, transparent, toneMapped, toneFrequency, name, loopVolumeCurveBehavior, delayRenderRetries, delayRenderTimeoutInMilliseconds, onVideoFrame, crossOrigin, audioStreamIndex, preservePitch: _preservePitch, ...props2 }) => {
	...
	const className$1 = (0, react.useMemo)(() => {
	...
	(0, react.useEffect)(() => {
		...
		const errorHandler = () => {
			if (current.error) {
				...
				if (onError) {
					onError(new MediaPlaybackError({
...
var SeriesSequenceInner = (0, react.forwardRef)(({ offset = 0, className: className$1 = "", _remotionInternalRender = null, ...props2 }, ref) => {
	useRequireToBeInsideSeries();
	if (_remotionInternalRender) return _remotionInternalRender({
		...props2,
		offset,
		className: className$1 || void 0
	}, ref);
...
function spring({ frame: passedFrame, fps, config = {}, from = 0, to = 1, durationInFrames: passedDurationInFrames, durationRestThreshold, delay: delay$1 = 0, reverse = false }) {
...
function interpolate$1(input, inputRange, outputRange, options) {
	...
	if (outputRange.some((output) => typeof output === "string")) {
		...
		try {
			...
			if (!outputRange.every((output) => typeof output === "string")) throw error$1;
			if (!outputRange.some((output) => {
				try {
...
function modernColorCall(name) {
	return /* @__PURE__ */ new RegExp(name + "\\(\\s*(" + MODERN_VALUE + ")\\s+(" + MODERN_VALUE + ")\\s+(" + MODERN_VALUE + ")(?:\\s*\\/\\s*(" + MODERN_VALUE + "))?\\s*\\)");
}
function getMatchers() {
	...
	if (cachedMatchers.rgb === void 0) {
		cachedMatchers.rgb = /* @__PURE__ */ new RegExp("rgb" + call(NUMBER, NUMBER, NUMBER));
		cachedMatchers.rgba = /* @__PURE__ */ new RegExp("rgba" + call(NUMBER, NUMBER, NUMBER, NUMBER));
		cachedMatchers.hsl = /* @__PURE__ */ new RegExp("hsl" + call(NUMBER, PERCENTAGE, PERCENTAGE));
		cachedMatchers.hsla = /* @__PURE__ */ new RegExp("hsla" + call(NUMBER, PERCENTAGE, PERCENTAGE, NUMBER));
		cachedMatchers.hex3 = /^#([0-9a-fA-F]{1})([0-9a-fA-F]{1})([0-9a-fA-F]{1})$/;
		cachedMatchers.hex4 = /^#([0-9a-fA-F]{1})([0-9a-fA-F]{1})([0-9a-fA-F]{1})([0-9a-fA-F]{1})$/;
		cachedMatchers.hex6 = /^#([0-9a-fA-F]{6})$/;
		cachedMatchers.hex8 = /^#([0-9a-fA-F]{8})$/;
...
var BufferingIndicator = ({ type, color = "white" }) => {
	const style$1 = type === "player" ? playerStyle : studioStyle;
...
var calculateOuterStyle = ({ config, style: style$1, canvasSize, overflowVisible, layout }) => {
	if (!config) return {};
	return {
		position: "relative",
...
var PlayerEmitter = class {
	constructor() {
		...
		this.dispatchError = (error$1) => {
			this.dispatchEvent("error", { error: error$1 });
		};
...
var usePlayerMethods = () => {
	...
	const seek$1 = (0, react.useCallback)((newFrame) => {
	...
	const play = (0, react.useCallback)((e) => {
		...
		if (getCurrentFrame() === lastFrameForPlayback) seek$1(0);
...
var useBrowserMediaSession = ({ browserMediaControlsBehavior, videoConfig, playbackRate }) => {
	...
	const { pause, play, emitter, getCurrentFrame, seek: seek$1 } = usePlayerMethods();
	...
	(0, react.useEffect)(() => {
		...
		navigator.mediaSession.setActionHandler("seekto", (event) => {
			if (browserMediaControlsBehavior.mode === "register-media-session" && event.seekTime !== void 0 && videoConfig) seek$1(Math.round(event.seekTime * videoConfig.fps));
		});
		navigator.mediaSession.setActionHandler("seekbackward", () => {
			if (browserMediaControlsBehavior.mode === "register-media-session" && videoConfig) seek$1(Math.max(0, Math.round((getCurrentFrame() - 10) * videoConfig.fps)));
		});
		navigator.mediaSession.setActionHandler("seekforward", () => {
			if (browserMediaControlsBehavior.mode === "register-media-session" && videoConfig) seek$1(Math.max(videoConfig.durationInFrames - 1, Math.round((getCurrentFrame() + 10) * videoConfig.fps)));
		});
		navigator.mediaSession.setActionHandler("previoustrack", () => {
			if (browserMediaControlsBehavior.mode === "register-media-session") seek$1(0);
		});
		return () => {
...
var usePlayback = ({ loop, playbackRate, moveToBeginningWhenEnded, inFrame, outFrame, browserMediaControlsBehavior, getCurrentFrame, muted }) => {
	...
	(0, react.useEffect)(() => {
		...
		const callback = () => {
			...
			if (nextFrame !== getCurrentFrame() && (!hasEnded || moveToBeginningWhenEnded) && !isBuffering()) setFrame((c$2) => ({
				...c$2,
				[config.id]: nextFrame
			}));
			if (hasEnded) {
...
var getHashOfDomain = async () => {
	...
	try {
		...
		return Array.from(new Uint8Array(hashBuffer)).map((b$2) => b$2.toString(16).padStart(2, "0")).join("");
	} catch {
		return null;
	}
};
...
var DefaultVolumeSlider = ({ volume, isVertical, onBlur, inputRef, setVolume }) => {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("style", { dangerouslySetInnerHTML: { __html: `
...
var getFrameFromX = (clientX, durationInFrames, width) => {
	const pos = clientX;
	return Math.round(interpolate(pos, [0, width], [0, durationInFrames - 1], {
		extrapolateLeft: "clamp",
		extrapolateRight: "clamp"
	}));
};
...
var PlayerSeekBar = ({ durationInFrames, onSeekEnd, onSeekStart, inFrame, outFrame }) => {
	...
	const { seek: seek$1, play, pause, isPlaying } = usePlayerMethods();
	const frame = Internals.Timeline.useTimelinePosition();
	...
	const onPointerMove = (0, react.useCallback)((e) => {
		...
		const posLeft = containerRef.current?.getBoundingClientRect().left;
		seek$1(getFrameFromX(e.clientX - posLeft, durationInFrames, size.width));
	}, [
		dragging.dragging,
		durationInFrames,
		seek$1,
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			style: barBackground,
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { style: (0, react.useMemo)(() => {
				return {
					height: BAR_HEIGHT2,
					backgroundColor: "rgba(255, 255, 255, 0.25)",
				...
				inFrame,
				outFrame
			]) }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { style: fillStyle })]
...
var Controls = ({ durationInFrames, isFullscreen, fps, showVolumeControls, onFullscreenButtonClick, allowFullscreen, onExitFullscreenButtonClick, spaceKeyToPlayOrPause, onSeekEnd, onSeekStart, inFrame, outFrame, initiallyShowControls, canvasSize, renderPlayPauseButton, renderFullscreenButton, alwaysShowControls, showPlaybackRateControl, containerRef, buffering, hideControlsWhenPointerDoesntMove, onPointerDown, onDoubleClick, renderMuteButton, renderVolumeSlider, playing, toggle, renderCustomControls }) => {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		ref,
		style: containerCss,
		onPointerDown: (0, react.useCallback)((e) => {
			if (e.target === ref.current || e.target === flexRef.current) onPointerDown?.(e);
		}, [onPointerDown]),
		onDoubleClick: (0, react.useCallback)((e) => {
...
var useClickPreventionOnDoubleClick = (onClick, onDoubleClick, doubleClickToFullscreen) => {
	...
	const handleClick = (0, react.useCallback)(async (e) => {
		...
		try {
			...
			const info$1 = errorInfo;
			api.removePendingPromise(waitForClick);
			if (!info$1.isCanceled) throw info$1.error;
		}
	}, [api, onClick]);
...
var PlayerUI = ({ controls, style: style2, loop, autoPlay, allowFullscreen, inputProps, clickToPlay, showVolumeControls, doubleClickToFullscreen, spaceKeyToPlayOrPause, errorFallback, playbackRate, renderLoading, renderPoster, className: className2, moveToBeginningWhenEnded, showPosterWhenUnplayed, showPosterWhenEnded, showPosterWhenPaused, showPosterWhenBuffering, showPosterWhenBufferingAndPaused, inFrame, outFrame, initiallyShowControls, renderFullscreen: renderFullscreenButton, renderPlayPauseButton, renderMuteButton, renderVolumeSlider, renderCustomControls, alwaysShowControls, showPlaybackRateControl, posterFillMode, bufferStateDelayInMilliseconds, hideControlsWhenPointerDoesntMove, overflowVisible, browserMediaControlsBehavior, overrideInternalClassName, noSuspense }, ref) => {
	const config = Internals.useUnsafeVideoConfig();
	const video = Internals.useVideo();
	const container$1 = (0, react.useRef)(null);
	const canvasSize = useElementSize(container$1, {
	...
	const requestFullscreen = (0, react.useCallback)(() => {
		...
		if (!container$1.current) throw new Error("No player ref found");
		if (container$1.current.webkitRequestFullScreen) container$1.current.webkitRequestFullScreen();
		else container$1.current.requestFullscreen();
	...
	(0, react.useEffect)(() => {
		...
		const fullscreenChange = () => {
			...
			if (element && element === container$1.current) player.emitter.dispatchFullscreenChange({ isFullscreen: true });
	...
	(0, react.useImperativeHandle)(ref, () => {
		const methods = {
			...
			toggle,
			getContainerNode: () => container$1.current,
			getCurrentFrame: player.getCurrentFrame,
	...
	const onError = (0, react.useCallback)((error$1) => {
		playerPause();
		playerDispatchError(error$1);
	...
	const content$1 = /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
		/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			style: outer,
...
var ThumbnailUI = ({ style: style2, inputProps, errorFallback, renderLoading, className: className2, overflowVisible, noSuspense, overrideInternalClassName }, ref) => {
	...
	const onError = (0, react.useCallback)((error$1) => {
		thumbnail.emitter.dispatchError(error$1);
	}, [thumbnail.emitter]);
	const loadingMarkup = (0, react.useMemo)(() => {
		return renderLoading ? renderLoading({
			height: outerStyle.height,
	...
	if (!config) return null;
	const content$1 = /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		style: outer,
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
			style: containerStyle3,
			className: playerCssClassname(overrideInternalClassName),
...
var Thumbnail = (0, react.forwardRef)(ThumbnailFn);
...
function mergeValues(a$2, b$2) {
	const aType = getParsedType(a$2);
	const bType = getParsedType(b$2);
	if (a$2 === b$2) return {
		valid: true,
		data: a$2
	};
	else if (aType === ZodParsedType.object && bType === ZodParsedType.object) {
		const bKeys = util.objectKeys(b$2);
		...
		for (const key of sharedKeys) {
			const sharedValue = mergeValues(a$2[key], b$2[key]);
		...
		if (a$2.length !== b$2.length) return { valid: false };
		const newArray = [];
		for (let index = 0; index < a$2.length; index++) {
			const itemA = a$2[index];
			const itemB = b$2[index];
	...
	} else if (aType === ZodParsedType.date && bType === ZodParsedType.date && +a$2 === +b$2) return {
		valid: true,
		data: a$2
	};
...
const FadeIn = ({ fade, children }) => {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AbsoluteFill, {
		style: { opacity: fade ? Math.min(1, frame / 12) : 1 },
		children
	});
};
		...
		children: [
			...
			timeline.overlays.map((ov, i) => {
				return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Sequence, {
					...
					durationInFrames: Math.max(1, SEC(fps, ov.endSeconds - ov.startSeconds)),
...
const NumberField = ({ label: label$2, value, step = .5, min = 0, onCommit }) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
	className: "djp-field",
	children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: label$2 }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
		type: "number",
		defaultValue: value,
		step,
		min,
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
										className: "djp-select",
										value: c$2.transition,
										onChange: (e) => o.updateClip(c$2.id, { transition: e.target.value }),
								...
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(NumberField, {
										label: "起点",
										value: c$2.inPoint,
										onCommit: (v) => o.updateClip(c$2.id, { inPoint: v })
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(NumberField, {
										label: "时长",
...
const FilmGlyph = ({ size = 16, className: className$1 }) => react.default.createElement("svg", {
	width: size,
	height: size,
	viewBox: "0 0 16 16",
	fill: "none",
